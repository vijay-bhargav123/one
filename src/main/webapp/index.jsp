<html>
<head>
        <title>My Java App</title>
</head>
<body>
       <h1 style="color: violet; font=size: 40px;" align="center">HEY WE DEPLOYED OUR APP USING JENKINS</h1>
       <h1 style="color: red; font=size: 40px;" align="center">THIS IS APP DEPLOYMENT USING TOMCAT</h1>
       <h1 style="color: blue; font=size: 50px;" align="center">Yadla Vijay bhargav updated this app</h1>
       <h1 style="color: blue; font=size: 50px;" align="center">im practicing jenkins app deployment with TOMCAT,NEXUS,MASTER-SLAVE CONECPTS in free style and pipeline </h1>

</body>
</html>

